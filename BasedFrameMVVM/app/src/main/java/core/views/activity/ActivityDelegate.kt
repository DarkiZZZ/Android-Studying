...
import core.sideeffects.SideEffectImplementationsHolder
...
class ActivityDelegate(
    ...
    private fun onResume() {
        sideEffectPluginsManager.plugins.forEach {
            activityViewModel.sideEffectMediatorHolder.setTargetWithPlugin(it, implementersHolder)
        }
    }

    @OnLifecycleEvent(Lifecycle.Event.ON_PAUSE)
    private fun onPause() {
        activityViewModel.sideEffectMediatorHolder.removeTargets()
    }
    ...
    private fun setupSideEffectMediator(plugin: SideEffectPlugin<*, *>) {
        val holder = activityViewModel.sideEffectMediatorHolder
    ...
    private fun setupSideEffectImplementer(plugin: SideEffectPlugin<*, *>) {
        implementersHolder.putWithPlugin(plugin, activityViewModel.sideEffectMediatorHolder, activity)
    }
